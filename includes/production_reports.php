...
function buildMonthlyProductionDetailedReportHtml(array $data, array $meta): string
{
    ...
    $html .= 'body{font-family:"Cairo","Segoe UI",Tahoma,Arial,sans-serif;background:#f8fafc;color:#0f172a;margin:0;padding:32px;width:100%;max-width:100%;overflow-x:hidden;}';
    $html .= '.report-container{max-width:1100px;margin:0 auto;background:#fff;border-radius:18px;box-shadow:0 25px 70px rgba(15,23,42,0.08);padding:32px;width:100%;max-width:100%;}';
    $html .= '.report-header{display:flex;flex-direction:column;gap:8px;margin-bottom:24px;width:100%;}';
    $html .= '.report-header h1{margin:0;font-size:28px;font-weight:700;color:#1e293b;word-wrap:break-word;}';
    $html .= '.report-header p{margin:0;font-size:15px;color:#64748b;word-wrap:break-word;}';
    $html .= '.meta-info{display:flex;flex-wrap:wrap;gap:12px;margin-top:8px;color:#475569;font-size:14px;}';
    $html .= '.meta-pill{background:#e2e8f0;border-radius:999px;padding:6px 16px;font-weight:500;white-space:nowrap;}';
    $html .= '.toolbar{display:flex;justify-content:flex-end;margin-bottom:20px;flex-wrap:wrap;gap:10px;}';
    $html .= '.toolbar button{background:#1d4ed8;color:#fff;border:none;border-radius:10px;padding:10px 18px;font-weight:600;cursor:pointer;transition:transform .2s ease,box-shadow .2s ease;white-space:nowrap;}';
    $html .= '.toolbar button:hover{transform:translateY(-1px);box-shadow:0 12px 24px rgba(29,78,216,0.18);}';
    $html .= '.stats-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:16px;margin-bottom:24px;width:100%;}';
    $html .= '.stat-card{background:linear-gradient(135deg,#eff6ff,#e0f2fe);border-radius:16px;padding:20px;position:relative;overflow:hidden;width:100%;max-width:100%;}';
    ...
    $html .= '.stat-card h3{margin:0 0 8px;font-size:16px;color:#0f172a;word-wrap:break-word;}';
    $html .= '.stat-card .value{font-size:26px;font-weight:700;color:#1d4ed8;word-wrap:break-word;}';
    $html .= '.section{margin-bottom:32px;width:100%;max-width:100%;}';
    $html .= '.section h2{margin:0 0 12px;font-size:22px;font-weight:700;color:#0f172a;word-wrap:break-word;}';
    $html .= '.section p{margin:0 0 16px;font-size:14px;color:#64748b;word-wrap:break-word;}';
    $html .= '.table-wrapper{background:#fff;border-radius:14px;overflow-x:auto;overflow-y:visible;box-shadow:0 12px 35px rgba(15,23,42,0.06);width:100%;max-width:100%;-webkit-overflow-scrolling:touch;}';
    $html .= '.table-wrapper::-webkit-scrollbar{height:8px;}';
    $html .= '.table-wrapper::-webkit-scrollbar-track{background:#f1f1f1;border-radius:4px;}';
    $html .= '.table-wrapper::-webkit-scrollbar-thumb{background:#888;border-radius:4px;}';
    $html .= '.table-wrapper::-webkit-scrollbar-thumb:hover{background:#555;}';
    $html .= 'table{width:100%;border-collapse:collapse;min-width:600px;table-layout:auto;}';
    $html .= 'thead{background:#0f172a;color:#fff;}';
    $html .= 'thead th{padding:14px;font-size:14px;font-weight:600;text-align:right;white-space:nowrap;position:sticky;top:0;z-index:10;}';
    $html .= 'tbody td{padding:12px 14px;border-bottom:1px solid #e2e8f0;font-size:14px;color:#334155;vertical-align:middle;word-wrap:break-word;}';
    ...
    $html .= '.badge{display:inline-flex;align-items:center;gap:6px;background:#e0f2fe;color:#0369a1;border-radius:999px;padding:6px 14px;font-size:13px;white-space:nowrap;}';
    $html .= '.chips{display:flex;flex-wrap:wrap;gap:10px;margin-bottom:16px;width:100%;}';
    $html .= '.chip{background:#e2e8f0;border-radius:999px;padding:6px 14px;font-size:13px;color:#1e293b;white-space:nowrap;}';
    $html .= '.chip strong{color:#0f172a;}';
    $html .= '@media screen and (max-width:768px){';
    $html .= 'body{padding:15px 10px;}';
    $html .= '.report-container{padding:20px 15px;border-radius:12px;}';
    $html .= '.report-header h1{font-size:22px;}';
    $html .= '.report-header p{font-size:14px;}';
    $html .= '.meta-info{gap:8px;font-size:13px;}';
    $html .= '.meta-pill{padding:5px 12px;font-size:12px;}';
    $html .= '.toolbar{justify-content:stretch;margin-bottom:15px;}';
    $html .= '.toolbar button{width:100%;padding:12px;}';
    $html .= '.stats-grid{grid-template-columns:1fr;gap:12px;margin-bottom:20px;}';
    $html .= '.stat-card{padding:15px;border-radius:12px;}';
    $html .= '.stat-card h3{font-size:14px;}';
    $html .= '.stat-card .value{font-size:22px;}';
    $html .= '.section{margin-bottom:25px;}';
    $html .= '.section h2{font-size:18px;margin-bottom:10px;}';
    $html .= '.section p{font-size:13px;margin-bottom:12px;}';
    $html .= '.table-wrapper{border-radius:12px;margin:0 -10px;width:calc(100% + 20px);max-width:calc(100% + 20px);}';
    $html .= 'table{min-width:500px;font-size:12px;}';
    $html .= 'thead th{padding:10px 8px;font-size:11px;}';
    $html .= 'tbody td{padding:10px 8px;font-size:11px;}';
    $html .= '.empty-state{padding:20px 15px;font-size:13px;}';
    $html .= '.badge{padding:5px 10px;font-size:12px;}';
    $html .= '.chips{gap:8px;margin-bottom:12px;}';
    $html .= '.chip{padding:5px 10px;font-size:12px;}';
    $html .= '}';
    $html .= '@media screen and (max-width:480px){';
    $html .= 'body{padding:10px 8px;}';
    $html .= '.report-container{padding:15px 12px;border-radius:10px;}';
    $html .= '.report-header h1{font-size:20px;}';
    $html .= '.report-header p{font-size:13px;}';
    $html .= '.meta-pill{padding:4px 10px;font-size:11px;}';
    $html .= '.stat-card{padding:12px;}';
    $html .= '.stat-card h3{font-size:13px;}';
    $html .= '.stat-card .value{font-size:20px;}';
    $html .= '.section h2{font-size:16px;}';
    $html .= 'table{min-width:450px;font-size:11px;}';
    $html .= 'thead th{padding:8px 6px;font-size:10px;}';
    $html .= 'tbody td{padding:8px 6px;font-size:10px;}';
    $html .= '.badge{font-size:11px;padding:4px 8px;}';
    $html .= '.chip{font-size:11px;padding:4px 8px;}';
    $html .= '}';
    $html .= '@media print{body{padding:0;background:#fff;} .toolbar{display:none;} .report-container{box-shadow:none;margin:0;border-radius:0;} .table-wrapper{overflow:visible;} table{min-width:auto;}}';
