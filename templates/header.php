    ...
    (function () {
        ...
        const applyHeader = (headers) => {
            ...
            if (Array.isArray(headers)) {
                ...
                for (let i = 0; i < headers.length; i++) {
                    if (Array.isArray(headers[i]) && headers[i][0] && headers[i][0].toLowerCase() === lowerKey) {
                        headers[i][1] = token;
                        updated = true;
                        break;
                    }
            ...
            const key = Object.keys(headers).find((headerKey) => headerKey.toLowerCase() === 'x-csrf-token');
            if (key) {
                headers[key] = token;
            } else {
        ...
        if (typeof window.fetch === 'function') {
            ...
            window.fetch = function (resource, init = {}) {
                if (resource && isSameOrigin(resource instanceof Request ? resource : resource)) {
                    if (resource instanceof Request) {
                        ...
                        return originalFetch(new Request(clonedRequest, requestInit));
                    }
            ...
            XMLHttpRequest.prototype.send = function (body) {
                if (this._csrfShouldAttach) {
                    try {
                        ...
                        // تجاهل أي أخطاء في حالة تم ضبط الرؤوس سابقاً
                    }
                }
